...
;
create index IDX_XXL_JOB_XXL_JOB_GROUP on XXL_JOB_INFO (XXL_JOB_GROUP)
;
...
;
create index IDX_XXL_JOB_LOG_XXL_JOB_GROUP on XXL_JOB_LOG (XXL_JOB_GROUP)
;
create index IDX_XXL_JOB_LOG_XXL_JOB_ID on XXL_JOB_LOG (XXL_JOB_ID)
;
...
comment on column XXL_JOB_LOGGLUE.update_time
  is '更新时间';
-- Create/Recreate indexes
create index IDX_XXL_JOB_LOGGLUE_XXL_JOB_ID on XXL_JOB_LOGGLUE (XXL_JOB_ID)
;
-- Create/Recreate primary, unique and foreign key constraints
...
comment on table XXL_JOB_LOG_REPORT is '任务报表';
...
create unique index IDX_XXL_JOB_LOG_REPORT_TRIGGER_DAY
...
-- Create sequence
create sequence XXL_JOB_REPORT_ID
minvalue 1
maxvalue 999999999999
start with 1
increment by 1
